...
//   const [allProducts, setAllProducts] = useState([...womenShawls]);
...
//           setAllProducts([...womenShawls, ...dbProducts]);
//         }
//       })
...
//       <img src={image11} alt="Kavi Shawls Banner" className="Customer_hero2-image w-100" />
...
//           <input
//             type="text"
//             placeholder="Search women's shawls..."
//             value={searchQuery}
...
//               <p className="text-muted fs-5">No women's shawls found matching your search.</p>
//             </div>
//           )}
//         </div>
//       </div>
...
//   const fabrics = ["All", ...new Set(allProducts.map((p) => p.fabric).filter((f) => f && f !== "N/A"))];
...
//   useEffect(() => {
//     setCurrentPage(1);
//   }, [searchQuery, selectedFabric, selectedColor, sortBy]);
...
//         {showFilters && (
//           <div className="row g-3 mt-2 p-3 bg-white border rounded-4 shadow-sm">
//             <div className="col-md-5">
//               <label className="form-label fw-bold small text-secondary">Fabric:</label>
//               <select
...
//             <div className="col-md-5">
//               <label className="form-label fw-bold small text-secondary">Color:</label>
//               <select
//                 className="form-select form-select-sm"
//                 value={selectedColor}
...
//           ) : (
//             <div className="text-center py-5">
//               <p className="text-muted fs-5">No women's shawls found matching your filters.</p>
//               <button
...
function Women() {
  ...
  const [selectedFabric, setSelectedFabric] = useState("All");
  const [selectedColor, setSelectedColor] = useState("All");
  const [maxPrice, setMaxPrice] = useState(5000); // Default max price slider value
  const [sortBy, setSortBy] = useState("newest");
  const [showFilters, setShowFilters] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);
  const productsPerPage = 12;
  ...
  const getProcessedProducts = () => {
    ...
    // Fabric Filter (supports checkboxes & 'All')
    if (selectedFabric !== "All") {
      list = list.filter((item) => item.fabric === selectedFabric);
    }
  ...
  return (
    <div className="Customer_container1">
      ...
      <div className="container my-3">
        ...
        {/* Collapsible Filter Panel matching the image design */}
        {showFilters && (
