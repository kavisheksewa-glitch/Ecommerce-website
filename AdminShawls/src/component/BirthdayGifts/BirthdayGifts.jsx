...
function BirthdayGifts() {
  ...
  const [allProducts, setAllProducts] = useState(() => formatStatic(BirthdayShawls));
  ...
  useEffect(() => {
    ...
    API.get("/api/seller/products/public")
      .then((res) => {
        ...
        setAllProducts([...formatStatic(BirthdayShawls), ...dbProducts]);
      })
      .catch((err) => {
        console.error("Error fetching live birthday products:", err);
        if (!cancelled) setAllProducts(formatStatic(BirthdayShawls));
      });
            ...
            >
              ...
              <div className="mb-3 border-bottom pb-3">
                ...
                <div className="form-check small mb-1">
                  ...
                  <label className="form-check-label">Birthday Hampers</label>
                </div>
              ...
              <div className="mb-3 border-bottom pb-3">
                <label className="fw-bold small mb-2 d-block" htmlFor="birthdayPriceRange">
                  Price Range
                </label>
                <input
                  id="birthdayPriceRange"
                  type="range"
                  ...
                  {MATERIAL_OPTIONS.map(({ label, match }) => (
                    <div className="form-check" key={match}>
                      <input
                        ...
                        type="checkbox"
                        id={`birthdayMaterial-${match}`}
                        checked={selectedFabrics.includes(match)}
                        onChange={() => toggleFabric(match)}
                      />
                      <label className="form-check-label" htmlFor={`birthdayMaterial-${match}`}>
                        {label}
                      </label>
                    </div>
                  ))}
            ...
            <div className="d-flex justify-content-between align-items-center mb-3 bg-white p-2 px-3 rounded shadow-sm border">
              <span className="text-muted small">
                {displayedGifts.length > 0
                  ? `Showing ${displayedGifts.length} product${displayedGifts.length > 1 ? "s" : ""}`
                  : "No products found"}
              </span>
              <select
              ...
              {displayedGifts.length > 0 ? (
                ...
                <div className="col-12 text-center py-5">
                  <p className="text-muted fs-5">
                    No birthday gift sets found matching your filters.
                  </p>
                  <button
                    type="button"
