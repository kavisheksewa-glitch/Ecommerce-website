...
function Checkout() {
  ...
  // ✅ FIX: productId ko sabse pehle priority do.
  // Home.jsx se aane wale product mein `id` field hoti hai.
  // Cart.jsx se aane wale item (Cart model) mein `id` nahi hoti — sirf
  // `productId` (asli product ka ID) aur `_id` (Cart document ka apna ID,
  // product ka NAHI) hoti hai. Pehle `productId` check karke ye mismatch fix
  // kiya gaya hai, warna Cart se Buy Now karne par galat ID (cart doc ki _id)
  // backend ko chali jaati thi aur "Product not found" error aata tha.
  ...
  const removeCartItemAfterOrder = async () => {
    try {
      ...
      if (data.success && Array.isArray(data.cart)) {
        const targetProductId = String(actualProductId);
        const cartItem = data.cart.find(
          (c) => String(c.productId) === targetProductId
        );
  ...
  const saveOrderToBackend = async ({ paymentStatus, razorpayPaymentId }) => {
    const response = await fetch("https://ecommerce-website-ggui.onrender.com/api/customer/order/create", {
      ...
      body: JSON.stringify({
        productId: actualProductId, // ✅ ab correct product ID jayega, Home ho ya Cart
        productTitle: product.title,
        productImage: product.image,
        price,
        quantity,
