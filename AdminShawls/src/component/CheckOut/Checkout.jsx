...
function Checkout() {
  ...
  // Price & Static Fees Calculations
  const price = Number(String(product.price).replace(/[₹,]/g, ""));
  const subtotal = price * quantity;
  
  const deliveryCharge = 60; // Static
  const platformFee = 10;    // Static
  const total = subtotal + deliveryCharge + platformFee;

  // Seller Amount Calculation:
  // Step 1: Minus ₹70 (60 + 10)
  const afterFeesDeduction = total - (deliveryCharge + platformFee);
  // Step 2: Minus 2% commission
  const finalSellerAmount = afterFeesDeduction * 0.98;
  ...
  const saveOrderToBackend = async ({ paymentStatus, razorpayPaymentId }) => {
    const response = await fetch("https://ecommerce-website-ggui.onrender.com/api/customer/order/create", {
      ...
      body: JSON.stringify({
        ...
        platformFee,
        totalAmount: total,
        sellerPayoutAmount: finalSellerAmount,
        ...
        <div className="col-lg-5 mb-4">
          <div className="card shadow border-0">
            ...
            <div className="card-body">
              ...
              <h4 className="text-success fw-bold">₹{price.toLocaleString()}</h4>
            </div>
          </div>
        </div>
            ...
            <form onSubmit={handleOrder}>
              ...
              <div className="mb-3">
                ...
                <div className="d-flex justify-content-between mb-2">
                  ...
                  <span className="fw-bold">₹{deliveryCharge}</span>
                </div>
                <div className="d-flex justify-content-between mb-2">
                  <span className="text-muted">Platform Fee:</span>
                  <span className="fw-bold">₹{platformFee}</span>
                </div>
              </div>
