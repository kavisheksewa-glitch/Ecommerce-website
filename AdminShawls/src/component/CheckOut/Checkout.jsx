...
//   // 🛒 Helper function: Order place hone ke baad cart se item hatane ke liye
//   const removeCartItemAfterOrder = async () => {
//     try {
//       const cartItemId = product._id || product.id;
//       await fetch(`http://localhost:5000/api/shawls/cart/remove/${cartItemId}`, {
//         method: "DELETE",
//       });
//     } catch (err) {
...
function Checkout() {
  ...
  const removeCartItemAfterOrder = async () => {
    try {
      ...
      if (data.success && Array.isArray(data.cart)) {
        ...
        if (cartItem) {
          await fetch(`http://localhost:5000/api/customer/cart/remove/${cartItem._id}`, {
            method: "DELETE",
            headers: { "Authorization": `Bearer ${token}` },
          });
  ...
  const saveOrderToBackend = async ({ paymentStatus, razorpayPaymentId }) => {
    const response = await fetch("http://localhost:5000/api/customer/order/create", {
      method: "POST",
      headers: {
