...
const createOrder = async (req, res) => {
  try {
    ...
    // ✅ FIX: sirf productId se dhoondo. sellerId ko query filter mein mat
    ...
    res.status(201).json({ success: true, message: "Order created successfully", order: savedOrder, remainingStock: updatedProduct.stockQuantity });
  } catch (error) {
    console.error("❌ Error in createOrder:", error.message);
    res.status(500).json({ success: false, message: error.message });
  }
